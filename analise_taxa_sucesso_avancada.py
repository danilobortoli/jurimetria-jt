...
# Diretórios
RESULTS_DIR = Path('resultados_taxa_sucesso_avancada')
RESULTS_DIR.mkdir(exist_ok=True)

class AnaliseTaxaSucessoAvancada:
    def __init__(self):
        # Caminho para os dados da tese
        self.data_path = Path("/Users/danilobortoli/Documents/Tese de doutorado da Andreia/Dados da segunda extração/raw")
    ...
    def load_all_data(self) -> List[Dict[str, Any]]:
        """Carrega todos os dados JSON da pasta raw"""
        print("Carregando todos os dados...")
        
        all_data = []
        
        # Carrega arquivos TRT
        trt_files = glob.glob(str(self.data_path / "trt*.json"))
        ...
        for file_path in trt_files:
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    print(f"Carregados {len(data)} registros de {Path(file_path).name}")
            except Exception as e:
                print(f"Erro ao carregar {file_path}: {e}")
        
        # Carrega arquivos TST
        tst_files = glob.glob(str(self.data_path / "tst*.json"))
        print(f"Encontrados {len(tst_files)} arquivos TST")
        ...
        print(f"Total de registros carregados: {len(all_data)}")
        return all_data
    ...
    def determine_instance(self, item: Dict[str, Any]) -> str:
        """Determina a instância com base nos dados"""
        tribunal = item.get('tribunal', '')
        
        if tribunal == 'TST':
            return 'TST'
        
        # Verifica se há códigos de movimento específicos
        movimentos = item.get('movimentos', [])
        for movimento in movimentos:
            codigo = movimento.get('codigo')
            if codigo in [219, 220]:  # Primeira instância
                return 'Primeira Instância'
            elif codigo in [237, 242, 236]:  # Segunda instância
                return 'Segunda Instância'
        
        # Fallback baseado no tribunal
        if 'TRT' in tribunal:
            return 'Segunda Instância'  # Assumindo que TRTs são segunda instância
        
        return 'Desconhecida'
    
    def determine_result(self, item: Dict[str, Any]) -> str:
        """Determina o resultado com base nos códigos e textos dos movimentos"""
        movimentos = item.get('movimentos', [])
        
        # Procura pelos códigos e textos específicos
        for movimento in movimentos:
            codigo = movimento.get('codigo')
            nome = movimento.get('nome', '').lower()
            # Códigos
            if codigo == 219 or 'procedente' in nome or 'procedência' in nome:
                return 'Procedente'
            elif codigo == 220 or 'improcedente' in nome or 'improcedência' in nome:
                return 'Improcedente'
            elif codigo == 237 or 'provido' in nome or 'provimento' in nome:
                return 'Provido'
            elif codigo in [242, 236] or 'desprovido' in nome or 'desprovimento' in nome or 'negado' in nome or 'negação de seguimento' in nome:
                return 'Desprovido'
        
        # Fallback para o campo resultado se existir
        return item.get('resultado', 'Desconhecido')
    ...
    def identify_appeal_chains(self, data: List[Dict[str, Any]]) -> List[List[Dict[str, Any]]]:
        ...
        for item in data:
            processed_item = {
                ...
                'numero_processo_norm': self.normalize_process_number(item.get('numeroProcesso', '')),
                'data_julgamento': item.get('data_julgamento', ''),
                'instancia': self.determine_instance(item),
                'resultado': self.determine_result(item),
                'movimentos': item.get('movimentos', []),
        ...
        for decisao in processed_data:
            numero_norm = decisao['numero_processo_norm']
            if numero_norm:
    ...
    def generate_report(self, estatisticas: Dict[str, Any]):
        ...
        report_path = self.results_dir / 'relatorio_taxa_sucesso_avancada.md'
        
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("# Relatório de Análise Avançada da Taxa de Sucesso de Recursos: Assédio Moral na Justiça do Trabalho\n\n")
            ...
            f.write("4. **Tags de movimento**: Análise baseada nos códigos de movimento da TPU/CNJ\n\n")
            ...
            f.write("4. **Códigos de movimento**: Alguns tribunais podem usar códigos alternativos para os mesmos movimentos.\n\n")
...
def main():
    analyzer = AnaliseTaxaSucessoAvancada()
